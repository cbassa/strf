...
int velocity(orbit_t orb, double mjd, struct site s, double *v, double *azi,
             double *alt);
...
void deselect_nearest(float x, float y, float xmin, float ymin, float xmax,
                      float ymax);
void save_data(float xmin, float ymin, float xmax, float ymax, char *filename);
void equatorial2horizontal(double mjd, struct site s, double ra, double de,
...
struct site get_site(int site_id) {
  ...
  while (fgets(line, LIM, file) != NULL) {
    ...
    // Read data
    status = sscanf(line, "%4d %2s %lf %lf %f", &id, abbrev, &lat, &lng, &alt);
...
void diagonal_select(float x0, float y0, float x1, float y1, int flag) {
  ...
  for (i = 0; i < d.n; i++) {
    ...
    ymin = y0 + v * (y1 - y0) - 3.0;
    ymax = y0 + v * (y1 - y0) + 3.0;
    if (v > 0.0 && v < 1.0 && d.p[i].f > ymin && d.p[i].f < ymax &&
        d.p[i].flag != 0)
      d.p[i].flag = flag;
  }
...
void format_tle(orbit_t orb, char *line1, char *line2) {
  ...
  // Print lines
  sprintf(line1, "1 %05dU          %2d%012.8f  .00000000  00000-0 %8s 0    0",
          orb.satno, orb.ep_year - 2000, orb.ep_day, sbstar);
  sprintf(line2, "2 %05d %8.4f %8.4f %07.0f %8.4f %8.4f %11.8f    0", orb.satno,
          DEG(orb.eqinc), DEG(orb.ascn), 1E7 * orb.ecc, DEG(orb.argp),
          DEG(orb.mnan), orb.rev);
...
void usage() {
  printf("dpplot -d <data file> -c [tle catalog] -i [satno] -h\n\ndata file:   "
         " Tabulated doppler curve\ntle catalog:  Catalog with TLE's "
         "(optional)\nsatno:        Satellite to load from TLE catalog "
         "(optional)\n\n");
...
int main(int argc, char *argv[]) {
  int i, j, flag, redraw = 1, plot_curve = 1, plot_type = 1, residuals = 0,
                  elset = 0;
  ...
  float dx[] = {0.1, 0.1, 0.35, 0.35, 0.6, 0.6, 0.85},
        dy[] = {0.0, -0.25, 0.0, -0.25, 0.0, -0.25, 0.0};
  int satno = -1, status;
  struct site s0, s1;
  ...
  for (;;) {
    ...
    // Change
    if (c == 'c') {
      printf("(1) Inclination,     (2) Ascending Node,   (3) "
             "Eccentricity,\n(4) Arg. of Perigee, (5) Mean Anomaly,     (6) "
             "Mean Motion,\n(7) B* drag,         (8) Epoch,            (9) "
    ...
    if (c == 'f') {
      ...
      if (satno < 0) {
        ...
        rms = fit_curve(orb, ia);
        // printf("rms: %.3f kHz, cf: %.6f MHz, TCA:
        // %s\n",rms,d.ffit/1000.0,nfd);
        printf("%05d %.3f %.3f %s %04d %02d%010.6f\n", orb.satno,
               d.ffit / 1000.0, rms, nfd, d.p[0].site_id, orb.ep_year - 2000,
               orb.ep_day);
        redraw = 1;
      }
    }
    ...
    // Save
    if (c == 'S') {
      printf("%s_%.3f_%04d_%05d.dat\n", nfd, d.ffit / 1000.0, d.p[0].site_id,
             satno);
    ...
    if (c == 't') {
      ...
      if (elset == 0) {
        orb.eqinc = 0.5 * M_PI;
        orb.ascn =
            modulo(gmst(0.5 * (d.mjdmin + d.mjdmax)) + site.lng, 360.0) * D2R;
        orb.ecc = 0.0001;
        orb.argp = 0.0;
...
struct point decode_line(char *line) {
  ...
  if (line[5] == '.') {
    site_id = -1;
    rsite_id = -1;
    nread = sscanf(line, "%lf %lf %f %d %d", &p.mjd, &p.freq, &p.flux, &site_id,
                   &rsite_id);
  } else {
    ...
    site_id = -1;
    status =
        sscanf(line, "%2d/%2d/%2d %2d:%2d:%2d.%1d %lf %f %d", &year, &month,
               &iday, &hour, &min, &sec, &fsec, &p.freq, &p.flux, &site_id);
...
struct data read_data(char *filename, int graves, float offset) {
  ...
  for (i = 1; i < d.n; i++) {
    if (d.p[i].flag > 0) {
      if (d.p[i].mjd < d.mjdmin)
        d.mjdmin = d.p[i].mjd;
      if (d.p[i].mjd > d.mjdmax)
        d.mjdmax = d.p[i].mjd;
      if (d.p[i].freq < d.freqmin)
        d.freqmin = d.p[i].freq;
      if (d.p[i].freq > d.freqmax)
        d.freqmax = d.p[i].freq;
      if (d.p[i].flux < d.fluxmin)
        d.fluxmin = d.p[i].flux;
      if (d.p[i].flux > d.fluxmax)
        d.fluxmax = d.p[i].flux;
    }
  }
...
double gmst(double mjd) {
  ...
  gmst = modulo(280.46061837 + 360.98564736629 * (mjd - 51544.5) +
...
void deselect_inside(float x0, float y0, float x, float y) {
  ...
  ymax = (y0 > y) ? y0 : y;
  for (i = 0; i < d.n; i++)
    if (d.p[i].t > xmin && d.p[i].t < xmax && d.p[i].f > ymin &&
        d.p[i].f < ymax && d.p[i].flag != 2)
      d.p[i].flag = 0;
    ...
    if (d.p[i].t > xmin && d.p[i].t < xmax && d.p[i].f > ymin &&
      ...
      if (d.p[i].rsite_id == 0)
        fprintf(file, "%lf\t%14.3lf\t%8.3f\t%04d\n", d.p[i].mjd,
                1000.0 * d.p[i].freq, d.p[i].flux, d.p[i].site_id);
      else
...
void equatorial2horizontal(double mjd, struct site s, double ra, double de,
  ...
  *azi = modulo(atan2(sin(h * D2R), cos(h * D2R) * sin(s.lat * D2R) -
                                        tan(de * D2R) * cos(s.lat * D2R)) *
                    R2D,
                360.0);
  *alt = asin(sin(s.lat * D2R) * sin(de * D2R) +
              cos(s.lat * D2R) * cos(de * D2R) * cos(h * D2R)) *
         R2D;
...
void mjd2nfd(double mjd, char *nfd) {
  ...
  sprintf(nfd, "%04d-%02d-%02dT%02d:%02d:%02.0f", year, month, day, hour, min,
          sec);
...
double doy2mjd(int year, double doy) {
  ...
  day =
      doy - floor(275.0 * month / 9.0) + k * floor((month + 9.0) / 12.0) + 30.0;
